...
The page is fluid. `--shell` is `100%` and the gutter (`clamp(20px, 4vw, 96px)`) is the margin, so the edge of the viewport is the edge of the sheet at every width rather than a fixed card floating in a dark room. Fluidity has exactly one cost — a row composed on twelve columns at 2560px puts a name at one edge and its action at the other with nothing trackable between — and the system pays it with `--row-max: 1240px` on the composed rows (`.bb-lead`, `.bb-role`, `.bb-sec-head`), the content width at 1440 where the composition was measured, so nothing at or below that width moves. The masthead is deliberately exempt: its display type is what the full width exists for.

Beneath that sits a twelve-column grid (`--cols`, `--col-gap`). Three regions are actually composed on it — lead rows, role rows and section heads — and they adopt it at 900px and collapse to one column below, because six columns at phone width is a grid you cannot compose in. The grid overlay (`app/components/grid.tsx`) draws those columns as red hairlines on demand, twelve above 720px and six below, and its state survives navigation and reload. It is the signature element and it is functional rather than decorative: a reader gets to check the structure instead of taking it on faith.
...
- Fluid page, capped rows (1240px), twelve columns revealed on demand.
...
### Masthead
Uppercase display name spanning the full grid, a 3px red rule directly beneath it spanning the same full width (a child of the masthead grid, never nested in a column), then the subhead and the meta list sharing the row below. Row gap is small and column gap large, deliberately: a uniform gap detached the rule from the name and pushed the email below the fold at 1440x900. The meta list is a two-column `<dl>` of 11px uppercase keys against 14px tabular-numeral values over a `--hair` top border.

### Composed Rows (`.bb-lead`, `.bb-role`, `.bb-sec-head`)
The three regions set on the twelve columns, all capped at `--row-max`. A lead row puts maturity in columns 1–2, the project name at lead scale across 3–9, the descriptor narrower beneath it across 3–8, and the action in 11–12; hover states rank in colour (name and action to red, bottom border to `--hair-strong`), never in movement. A role row is the same skeleton: years in 1–2, role and company in 3–9, status chip in 11–12. A section head is flex at every width — heading, a `flex: 1` hairline that runs from the last letter to the stamp, then the stamp — because placed on the columns the rule floated detached in the middle of the row.
...
- **Don't** put the masthead's red rule inside a column, and don't cap the masthead at `--row-max`; it is exempt on purpose.
